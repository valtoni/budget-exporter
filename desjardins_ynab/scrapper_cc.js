(() => {
    ...
    tables.forEach(dsdTable => {
        ...
        rows.forEach(row => {
            ...
            const amountTd = [...tds]
                .filter(td => td.classList.contains('dsd-cell-num'))
                .pop(); // pega o último com essa classe

            const amountRaw = amountTd?.innerText.trim() || '';


            const amount = amountRaw.replace(/\s/g, '').replace('$', '').replace(/\./g, '').replace(',', '.');
            const isInflow = amount.startsWith('+');
            const inflow = isInflow ? amount.replace('+', '') : '';
            const outflow = !isInflow ? amount.replace('-', '') : '';
